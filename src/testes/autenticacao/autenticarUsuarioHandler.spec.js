...
describe('autenticacao.business.service', () => {
    test.each`
    usuario         | senha         | retornoEsperado
    ${""}	        | ${""}         | ${true}
    ${null}	        | ${null}	    | ${true}
    ${undefined}    | ${undefined}  | ${true}
    `("ao invocar handler | quando usuarioIdenticacao ou usuarioSenha inválidos | deve retornar erro",
        async ({ usuario, senha, retornoEsperado }) => {
            const { sut } = criarSUT();
